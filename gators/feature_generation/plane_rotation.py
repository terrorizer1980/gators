...
class PlaneRotation(Transformer):
    ...
    * and `pandas` dataframes:
    ...
    def fit(self, X: DataFrame, y: Series = None) -> "PlaneRotation":
        ...
        usecols = list(set(self.columns))
        self.check_dataframe_is_numerics(X[usecols])
        self.idx_columns_x = util.get_idx_columns(X[usecols], self.columns[::2])
        self.idx_columns_y = util.get_idx_columns(X[usecols], self.columns[1::2])
        self.idx_subarray = util.get_idx_columns(X.columns, usecols)
